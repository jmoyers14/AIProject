...
public class Checkers extends JPanel {
    ...
    public Checkers() {
        ...
        setPreferredSize(new Dimension(350 * mult, 250 * mult));
        ...
        board.setBounds(20, 20, 164 * mult, 164 * mult); // Note:  size MUST be 164-by-164 !
        newGameButton.setBounds(210 * mult, 60 * mult, 120, 30);
        resignButton.setBounds(210 * mult, 120 * mult, 120, 30);
        ComputerButton.setBounds(210 * mult, 180 * mult, 120, 30);
        message.setBounds(0, 200 * mult, 350, 30);
...
//            AIboard = new CheckersData();
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (src == newGameButton) {
                ...
                doResign();
            } else if (src == ComputerButton) {
                doAuto();
            }
        }

        void doAuto() {  //change below to work with miniMax
//            message.setText("do AI");
            if (currentPlayer == CheckersData.BLACK) {
                if (legalMoves.length ==1){
                    doMakeMove(legalMoves[0],board,true);
                }else{
//                Random generator = new Random();
//                int i=Math.abs(generator.nextInt()%legalMoves.length);
//                message.setText("temp is "+i);
//                CheckersMove move=legalMoves[i];
//                doMakeMove(move,board,true);
//                board.makeMove(move.toRow,move.toCol,move.fromRow, move.fromCol,currentPlayer);
...
//                repaint();
                ...
                AIboard = new CheckersData();
                AIboard = board;
                int temp=alphabeta(currentPlayer, AIboard, 1, -1,Depth);
                message.setText("temp is "+temp);
                legalMoves = board.getLegalMoves(currentPlayer);
                doMakeMove(legalMoves[temp],board,true);
//doMakeMove(legalMoves[temp], board,true);
//                board = AIboard;
                repaint();
                }
        ...
        int alphabeta(int player, CheckersData myBoard, int alpha, int beta, int depth) {
            if (myBoard.getLegalMoves(player) == null || depth==0) {
...
//                      gameOver("RED has no moves.  BLACK wins.");
            ...
            int index=0;
            CheckersMove[] children = myBoard.getLegalMoves(player);
            if (player == CheckersData.BLACK) {
                for (int i=0;i < children.length;i++) {
                    doMakeMove(children[i], myBoard,false);
                    int score = alphabeta(CheckersData.RED, myBoard, myMax(), myMin(),depth-1);
                    myBoard.makeMove(children[i].toRow,children[i].toCol, children[i].fromRow, children[i].fromCol,1);
                    currentPlayer = CheckersData.BLACK;
                    if (score > alpha) {
                        index=i;
                ...
                for (int i=0;i < children.length;i++) {
                    doMakeMove(children[i],myBoard,false);
                    int score = alphabeta(CheckersData.BLACK, myBoard, myMax(), myMin(),depth-1);
                    myBoard.makeMove(children[i].toRow,children[i].toCol, children[i].fromRow, children[i].fromCol,3);
        ...
        int myMax(){
            ...
            return black -red;//Math.max(red, black);
        }
        ...
        int myMin(){
            ...
            return red-black;// Math.min(red, black);
        }
...
//            AIboard.setUpGame();
        ...
        void doClickSquare(int row, int col) {
            ...
            for (int i = 0; i < legalMoves.length; i++) {
                if (legalMoves[i].fromRow == selectedRow && legalMoves[i].fromCol == selectedCol
                    ...
                    doMakeMove(legalMoves[i],board,true);
                    doAuto();
                    return;
                }
            }
         ...
         * appropriately.
         */
        void doMakeMove(CheckersMove move,CheckersData myBoard,boolean paint) {

            myBoard.makeMove(move,0);
             ...
             */
            if (move.isJump()) {
                legalMoves = myBoard.getLegalJumpsFrom(currentPlayer, move.toRow, move.toCol);
                if (legalMoves != null) {
                    if (currentPlayer == CheckersData.RED) {
            ...
            if (currentPlayer == CheckersData.RED) {
                ...
                legalMoves = myBoard.getLegalMoves(currentPlayer);
                if (legalMoves == null) {
                     message.setText("BLACK has no moves.  RED wins.");
//                    gameOver("BLACK has no moves.  RED wins.");
                ...
                legalMoves = myBoard.getLegalMoves(currentPlayer);
                if (legalMoves == null) {
                    message.setText("RED has no moves.  BLACK wins.");
//                    gameOver("RED has no moves.  BLACK wins.");
                } else if (legalMoves[0].isJump()) {
        ...
        public void paintComponent(Graphics g) {
            ...
            g.drawRect(0, 0, mult * (getSize().width - 1), mult * (getSize().height - 1));
            g.drawRect(1, 1, mult * (getSize().width - 3), mult * (getSize().height - 3));
            ...
            for (int row = 0; row < 8; row++) {
                for (int col = 0; col < 8; col++) {
                    ...
                    g.fillRect((2 + col * 20) * mult, (2 + row * 20) * mult, 20 * mult, 20 * mult);
                    switch (board.pieceAt(row, col)) {
                        case CheckersData.RED:
                            g.setColor(Color.RED);
                            g.fillOval((4 + col * 20) * mult, (4 + row * 20) * mult, 15 * mult, 15 * mult);
                            break;
                        ...
                        case CheckersData.RED_KING:
                            ...
                            g.drawString("K", (7 + col * 20) * mult, (16 + row * 20) * mult);
                            break;
            ...
            if (gameInProgress) {
                ...
                for (int i = 0; i < legalMoves.length; i++) {
                    g.drawRect((2 + legalMoves[i].fromCol * 20) * mult, (2 + legalMoves[i].fromRow * 20) * mult, 19 * mult, 19 * mult);
                    g.drawRect((3 + legalMoves[i].fromCol * 20) * mult, (3 + legalMoves[i].fromRow * 20) * mult, 17 * mult, 17 * mult);
                }
                ...
                if (selectedRow >= 0) {
                    g.setColor(Color.white);
                    g.drawRect((2 + selectedCol * 20) * mult, (2 + selectedRow * 20) * mult, 19 * mult, 19 * mult);
                    g.drawRect((3 + selectedCol * 20) * mult, (3 + selectedRow * 20) * mult, 17 * mult, 17 * mult);
                    g.setColor(Color.green);
                    for (int i = 0; i < legalMoves.length; i++) {
                        if (legalMoves[i].fromCol == selectedCol && legalMoves[i].fromRow == selectedRow) {
                            g.drawRect((2 + legalMoves[i].toCol * 20) * mult, (2 + legalMoves[i].toRow * 20) * mult, 19 * mult, 19 * mult);
                            g.drawRect((3 + legalMoves[i].toCol * 20) * mult, (3 + legalMoves[i].toRow * 20) * mult, 17 * mult, 17 * mult);
                        }
                    }
                }
            }
        ...
        public void mousePressed(MouseEvent evt) {
            if (gameInProgress == false) {
                ...
                int col = ((evt.getX() - 2) / 20) / mult;
                int row = ((evt.getY() - 2) / 20) / mult;
    ...
    private static class CheckersData {
         ...
         */
        void makeMove(CheckersMove move,int player) {
            makeMove(move.fromRow, move.fromCol, move.toRow, move.toCol,player);
        }
        ...
        void makeMove(int fromRow, int fromCol, int toRow, int toCol,int player) {
            ...
            if (Math.abs(fromRow - toRow) == 2 || Math.abs(fromRow - toRow) == 2) {
                // The move is a jump.  Remove the jumped piece from the board.
                int jumpRow = (fromRow + toRow) / 2;  // Row of the jumped piece.
                int jumpCol = (fromCol + toCol) / 2;  // Column of the jumped piece.
                board[jumpRow][jumpCol] = player;
            }
            if (toRow == 0 && board[toRow][toCol] == RED && player==0) {
                board[toRow][toCol] = RED_KING;
            }
            if (toRow == 7 && board[toRow][toCol] == BLACK && player==0) {
